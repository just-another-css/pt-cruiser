...
static void parse_input(int* num_objects, PointsMesh** mesh, RenderParameters* params, CameraPath** camera_path) {
    ...
    *mesh = (PointsMesh*) malloc(scene.obj_len * sizeof(PointsMesh));
    MALLOC_CHECK(*mesh);
    for (int i = 0; i < scene.obj_len; i++) {
        int num_triangles = scene.objects[i].faces->len;
        int num_vertices = scene.objects[i].points->len;
        ...
        float2 object_uvs[3];
        float value;
        for (int a = 0; a < scene.objects[i].desc_args->len; a++) {
            switch (scene.objects[i].desc_args->args[a].type) {
                case MATERIAL_DESC_ARG:
                    object_material = find_material(scene.objects[i].desc_args->args[a].material, material_names, scene.mat_len);
                ...
                case LIGHTING_DESC_ARG:
                    value = scene.objects[i].desc_args->args[a].lighting;
                    object_lighting = make_float3(value, value, value);
                    object_lighting_set = true;
                    break;
                case UV_DESC_ARG:
                    object_uvs[0] = make_float2(scene.objects[i].desc_args->args[a].uvs->list[0].x,
                                                scene.objects[i].desc_args->args[a].uvs->list[0].y);
                    object_uvs[1] = make_float2(scene.objects[i].desc_args->args[a].uvs->list[1].x,
                                                scene.objects[i].desc_args->args[a].uvs->list[1].y);
                    object_uvs[2] = make_float2(scene.objects[i].desc_args->args[a].uvs->list[2].x,
                                                scene.objects[i].desc_args->args[a].uvs->list[2].y);
                    object_uvs_set = true;
                    break;
            }
        }
        for (int tri = 0; tri < num_triangles; tri++) {
            ...
            for (int a = 0; a < scene.objects[i].faces->faces[tri].desc_args->len; a++) {
                switch (scene.objects[i].faces->faces[tri].desc_args->args[a].type) {
                    case MATERIAL_DESC_ARG:
                        (*mesh)[i].materials[tri] = find_material(scene.objects[i].faces->faces[tri].desc_args->args[a].material, material_names, scene.mat_len);
                    ...
                    case UV_DESC_ARG:
                        ...
                        (*mesh)[i].uv[tri * 3] = make_float2(scene.objects[i].faces->faces[tri].desc_args->args[a].uvs->list[0].x,
                                                             scene.objects[i].faces->faces[tri].desc_args->args[a].uvs->list[0].y);
                        (*mesh)[i].uv[tri * 3 + 1] = make_float2(scene.objects[i].faces->faces[tri].desc_args->args[a].uvs->list[1].x,
        ...
        for (int v = 0; v < num_vertices; v++) {
            Vec_t now_v = scene.objects[i].points->list[v];
    ...
    for (int i = 0; i < scene.param_len; i++) {
        valid = true;
        ...
        if (!valid) {
            fprintf(stderr, "[!] Parameter #%d '%s' with value %f/(%f,%f,%f) was not recognised\n", i, scene.params[i].name, scene.params[i].num_value, scene.params[i].vec_value.x, scene.params[i].vec_value.y, scene.params[i].vec_value.z);
            exit(EXIT_FAILURE);
        }
    }
    ...
    if (scene.path_set) {
        *camera_path = (CameraPath*) malloc(sizeof(CameraPath));
        CameraPathNode* pos_node = scene.path.lists[POS_LIST];
        if (pos_node) {
            (*camera_path)->pos_path_end = (*camera_path)->pos_path_start = (*camera_path)->pos_path = (PositionPathNode*) malloc(sizeof(PositionPathNode));
            ...
            (*camera_path)->pos_path_end = (*camera_path)->pos_path_start = (*camera_path)->pos_path = NULL;
        }
        CameraPathNode* pitch_node = scene.path.lists[PITCH_LIST];
        if (pitch_node) {
            (*camera_path)->pitch_path_end = (*camera_path)->pitch_path_start = (*camera_path)->pitch_path = (PitchPathNode*) malloc(sizeof(PitchPathNode));
            ...
            (*camera_path)->pitch_path_end = (*camera_path)->pitch_path_start = (*camera_path)->pitch_path = NULL;
        }
        CameraPathNode* yaw_node = scene.path.lists[YAW_LIST];
        if (yaw_node) {
            (*camera_path)->yaw_path_end = (*camera_path)->yaw_path_start = (*camera_path)->yaw_path = (RotationPathNode*) malloc(sizeof(RotationPathNode));
            ...
            (*camera_path)->yaw_path_end = (*camera_path)->yaw_path_start = (*camera_path)->yaw_path = NULL;
        }
        CameraPathNode* roll_node = scene.path.lists[ROLL_LIST];
        if (roll_node) {
            (*camera_path)->roll_path_end = (*camera_path)->roll_path_start = (*camera_path)->roll_path = (RotationPathNode*) malloc(sizeof(RotationPathNode));
            ...
            (*camera_path)->roll_path_end = (*camera_path)->roll_path_start = (*camera_path)->roll_path = NULL;
        }
        if (scene.path.fps) params->cam_path_framerate = scene.path.fps;
    } else {
        *camera_path = NULL;
    }
