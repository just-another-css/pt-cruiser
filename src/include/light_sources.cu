...
__global__ static void calculate_triangle_powers(int* cur_lit_face_is) {
    ...
    light_sources_dev.light_face_is[i][face_i] = lit_face_i;
...
void initialise_light_sources(int num_objects, PointsMesh* meshes, int* all_num_lit_faces) {
    ...
    int* light_object_is = (int*) malloc(num_light_sources * sizeof(int));
    int* light_is = (int*) malloc(num_objects * sizeof(int));
    ...
    int** light_face_is_devs = (int**) malloc(num_light_sources * sizeof(int*));
    ...
    for (int i = 0, j = 0; j < num_light_sources; i++) {
        if (all_num_lit_faces[i]) { 
            ...
            CUDA_CHECK(cudaMalloc(&light_face_is_devs[j], meshes[i].triangle_count * sizeof(int)));
    ...
    CUDA_CHECK(cudaMalloc(&light_sources_dev_cpy.light_is, num_objects * sizeof(int)));
    ...
    CUDA_CHECK(cudaMalloc(&light_sources_dev_cpy.light_face_is, num_light_sources * sizeof(int*)));
    ...
    CUDA_CHECK(cudaMemcpy(light_sources_dev_cpy.obj_is, light_object_is, num_light_sources * sizeof(int), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(light_sources_dev_cpy.light_is, light_is, num_objects * sizeof(int), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(light_sources_dev_cpy.light_face_is, light_face_is_devs, num_light_sources * sizeof(int*), cudaMemcpyHostToDevice));
    ...
    free(light_object_is);
    free(light_is);
    free(light_source_num_lit_faces);
    free(face_is_devs);
    free(light_face_is_devs);
